...
function parseCandidateText(text: string) {
  ...
  return {
    ...
    preferred_job_type: (get('Preferred Job Type') || get('Job Type')).toLowerCase().replace(/\s+/g, '_'),
    expected_hourly_rate: parseFloat(get('Expected Hourly Rate') || get('Hourly Rate')) || undefined,
    experience_years: parseFloat(get('Experience Years') || get('Experience')) || undefined,
...
function parseJobText(text: string) {
  ...
  return {
    ...
    company: get('Company'),
    location: get('Location'),
    job_type: (get('Job Type') || get('Type')).toLowerCase().replace(/\s+/g, '_'),
    job_subtype: (get('Sub Type') || get('Subtype') || get('Job Subtype')).toLowerCase().replace(/[\s-]+/g, '_'),
    ...
    pay_per_hour: parseFloat(get('Pay Per Hour') || get('Hourly Pay')) || undefined,
    skills_required: (get('Skills Required') || get('Skills') || get('Required Skills'))
      .split(',')
      .map((s) => s.trim())
      .filter(Boolean),
    experience_required: parseFloat(get('Experience Required') || get('Required Experience')) || undefined,
  ...
  if (parsed) {
    return (
      <div className="paste-preview">
        ...
        {isCandidate ? (
          <CandidateForm initialData={parsed} onSubmit={handleSave} submitLabel="Save Candidate" />
        ) : (
          <JobForm initialData={parsed} onSubmit={handleSave} submitLabel="Save Job" />
        )}
      </div>
    );
  }
  ...
  return (
    <div className="paste-tab">
      <p className="tab-description">
        Paste {isCandidate ? 'candidate' : 'job'} details using the <strong>Key: Value</strong> format
        below. Fields will be automatically extracted and shown for review before saving.
      </p>
      <textarea
      ...
      <div className="paste-actions">
        ...
        <button
          ...
          onClick={() => setText(isCandidate ? CANDIDATE_TEMPLATE : JOB_TEMPLATE)}
        >
          Load Example
        </button>
        {text && (
