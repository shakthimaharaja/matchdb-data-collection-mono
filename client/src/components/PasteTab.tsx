...
function parseJobText(text: string) {
  ...
  const stripEmojis = (s: string) =>
    s
      .replace(
        /[\u{1F300}-\u{1FAFF}\u{2600}-\u{27BF}\u{FE00}-\u{FE0F}\u{200D}\u{2702}-\u{27B0}]/gu,
        "",
      )
      .trim();
  ...
  if (roleMatch) {
    ...
    title = stripEmojis(lines[0])
      ...
      .replace(/\s*\|.*$/, "")
      .replace(
        /^(?:hello|hi|hey)[,!.]?\s*(?:accepting\s+resumes?\s+(?:for\s+)?(?:below\s+)?role)?/i,
        "",
      )
      .trim();
    // if first line was just greeting, try second line
    if (!title && lines.length > 1) {
      title = stripEmojis(lines[1])
        .replace(/^[-–—:]\s*/, "")
        .trim();
    }
  }
  ...
  location = location
    .replace(/\s*\([^)]*(?:local|only|required|dl|drivers?)\b[^)]*\)/gi, "")
    .trim();
  ...
  let work_mode = "";
  if (
    /\bhybrid\s*(?:role)?\b/i.test(full) ||
    /\(\d+\s*days?\s*onsite\)/i.test(full)
  )
    work_mode = "hybrid";
  else if (
    ...
    work_mode = "onsite";
  else if (/\b(remote|wfh|work\s*from\s*home|telecommute)\b/i.test(full))
    work_mode = "remote";
  ...
  const hourlyMatch = full.match(
    /\$\s*([\d,.]+)\s*(?:\/|\s*per\s*)\s*h(?:ou)?r/i,
  );
  ...
  for (const regex of skillBlockPatterns) {
    ...
    while ((m = regex.exec(full)) !== null) {
      ...
      const tokens = content
        .split(/[,;•|\n]+/)
        .flatMap((t) => t.split(/\s*\+\s*/).map((s) => s.trim()));
  ...
  let company = matchAny(/(?:company|client|employer)\s*[:=]\s*([^\n|]+)/i);
  if (!company) {
  ...
  const recruiter_email =
    ...
    matchAny(/^\s*([\w.+-]+@[\w.-]+)\s*$/m);
  const recruiter_phone = matchAny(
  ...
  const descLines = lines
    ...
    .filter(
      (l) =>
        !l.startsWith("📍") &&
        !/^(?:recruiter|contact|poc|email|phone)\s*[:=]/i.test(stripEmojis(l)),
    );
