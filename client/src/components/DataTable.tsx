...
export default function DataTable({ records, type, onDelete }: Props) {
    ...
    return (
      <div className="data-table-section">
        ...
        <div className="empty-state">
          ...
          <p>No records yet</p>
          <p className="empty-hint">Start by adding data using the tabs above.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="data-table-section">
      <div className="section-header">
        <h3>
          Recent Records <span className="record-count">({records.length})</span>
        </h3>
      </div>
                  ...
                  <>
                    ...
                    <td>
                      <span className="skills-cell">
                        ...
                        {r.skills?.length > 3 && <span className="more-badge">+{r.skills.length - 3}</span>}
                      </span>
                    </td>
                    <td>{r.experience_years ? `${r.experience_years}y` : '—'}</td>
                  </>
                ) : (
                  <>
                    ...
                    <td>{r.company}</td>
                    <td>{r.location || '—'}</td>
                    <td className="cell-type">{r.job_type?.replace(/_/g, ' ') || '—'}</td>
                    <td>
                      <span className="skills-cell">
                        {r.skills_required?.slice(0, 3).join(', ')}
                        {r.skills_required?.length > 3 && <span className="more-badge">+{r.skills_required.length - 3}</span>}
                      </span>
                    </td>
                  </>
                )}
                <td>
                  <span className={`badge ${SOURCE_CONFIG[r.source]?.cls || ''}`}>
