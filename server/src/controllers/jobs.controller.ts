...
export async function createJob(req: AuthRequest, res: Response) {
  try {
    ...
    res.status(400).json({ error: err.message || 'Failed to create job record' });
  }
}
...
export async function uploadExcel(req: AuthRequest, res: Response) {
  try {
    ...
    const normalize = (val: string) => val.toLowerCase().replace(/[\s-]+/g, '_');

    const jobs = rows
      .map((row) => ({
        title: row.title || row.Title || row['Job Title'] || '',
        description: row.description || row.Description || row['Job Description'] || '',
        company: row.company || row.Company || '',
        location: row.location || row.Location || '',
        job_type: normalize(row.job_type || row['Job Type'] || row.Type || 'full_time'),
        job_subtype: normalize(row.job_subtype || row['Sub Type'] || row.Subtype || ''),
        work_mode: normalize(row.work_mode || row['Work Mode'] || row.Mode || ''),
        salary_min: parseFloat(row.salary_min || row['Salary Min'] || row['Min Salary'] || '0') || undefined,
        salary_max: parseFloat(row.salary_max || row['Salary Max'] || row['Max Salary'] || '0') || undefined,
        pay_per_hour: parseFloat(row.pay_per_hour || row['Pay Per Hour'] || row['Hourly Pay'] || '0') || undefined,
        skills_required:
          typeof (row.skills_required || row['Skills Required'] || row.Skills) === 'string'
            ? (row.skills_required || row['Skills Required'] || row.Skills).split(',').map((s: string) => s.trim()).filter(Boolean)
            : [],
        experience_required:
          parseFloat(row.experience_required || row['Experience Required'] || row.Experience || '0') || undefined,
        recruiter_name: row.recruiter_name || row['Recruiter Name'] || row.Recruiter || '',
    ...
    if (jobs.length === 0) {
      res.status(400).json({ error: 'No valid rows found. Ensure "title" and "company" columns exist.' });
      return;
    }
